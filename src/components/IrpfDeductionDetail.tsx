...
export default function IrpfDeductionDetail({
  data,
  darkMode,
}: IrpfDeductionDetailProps) {
  if (
      ...
      {(data.familyDetail?.childDeduction ?? 0) > 0 && (
        ...
        >
          ...
          <span className="text-green-400">
            -{formatUyu(data.familyDetail?.childDeduction ?? 0)}
          </span>
        </div>
      )}
      {(data.familyDetail?.disabledChildDeduction ?? 0) > 0 && (
        ...
        >
          ...
          <span className="text-green-400">
            -{formatUyu(data.familyDetail?.disabledChildDeduction ?? 0)}
          </span>
        </div>
      )}
    </div>
