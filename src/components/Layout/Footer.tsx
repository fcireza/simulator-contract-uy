...
export default function Footer({ darkMode, onNavigate }: FooterProps) {
  ...
  return (
    <footer className={`bottom-0 left-0 right-0 ${darkMode ? 'bg-gray-900/95 backdrop-blur-sm' : 'bg-white/95 backdrop-blur-sm'} shadow-sm transition-colors duration-200`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col items-center py-4 sm:flex-row sm:justify-between sm:items-start gap-4">
          ...
          <div className="text-center sm:text-left order-2">
            <div className="flex flex-col items-center sm:items-start gap-1">
              <p style={{ fontWeight: 'bold' }}>Enlaces Rápidos</p>
              <button
                onClick={() => handleNavigate('simulator')}
                className={`${linkClass} text-sm text-left`}
              >
                Simulador
              </button>
              <button
        ...
        <div className="border-t border-border pt-2 py-5 text-center text-muted-foreground text-[0.85rem]">
          <p>&copy; {currentYear}  💻 Construido con ❤️ por 
            <a
             href="https://linkedin.com/in/federico-cireza/"
              ...
              aria-label="LinkedIn">
                fcireza |
                <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" className="inline-block align-text-bottom ml-2 mr-1">
