...
function renderDimensions(
  ...
  return (
    <div className="space-y-4">
      {items.map((item) => (
        <div key={item.label} className={`p-3 rounded-lg ${darkMode ? 'bg-gray-700/30' : 'bg-gray-50'}`}>
          <h5
...
function renderProsCons(pros: string[], cons: string[], darkMode: boolean) {
  return (
...
export default function Guide() {
  ...
  return (
    <div className={`max-w-5xl mx-auto ${textClass}`}>
        ...
        {categories.map((category) => (
          <div key={category.id} className="mb-8">
            ...
            <div className="space-y-3">
              ...
              {category.id === 'intro' && (
                <div className={`rounded-xl border transition-all duration-200 ${cardClass}`}>
                  <button
                  ...
                  {expandedSection === 'intro-card' && (
                    <div className="px-6 pb-5 pt-0">
                      <p className="leading-relaxed mb-4 whitespace-pre-line">{guideIntroContent}</p>
                      <p
                        className={`text-xs border-t pt-3 ${darkMode ? 'text-gray-500 border-gray-700' : 'text-gray-400 border-gray-200'}`}
                      >
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <span className={`font-semibold text-lg ${headingClass}`}>{sd.name}</span>
                      </div>
                      <svg
              ...
              {category.id === 'recomendaciones' && (
                <div className="space-y-4">
                  ...
                  >
                    <h4 className={`font-bold text-base mb-4 ${headingClass}`}>Uso según perfil</h4>
                    <div className="space-y-3">
                      {industryRecommendations.map((rec) => (
                        <div key={rec.level} className={`p-4 rounded-lg ${darkMode ? 'bg-gray-700/30' : 'bg-gray-50'}`}>
                          <div className="flex items-start justify-between gap-4">
                            <div>
                              <h5 className={`font-semibold text-sm ${headingClass}`}>{rec.level}</h5>
                  ...
                  <div
                    className={`rounded-xl border p-6 ${darkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200'}`}
                  >
                    <h4 className={`font-bold text-base mb-4 ${headingClass}`}>Regla práctica según ingreso (2026)</h4>
                    <div className="overflow-x-auto">
                      <table
                        className={`w-full text-sm border-collapse ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}
                      >
                        <thead>
                          <tr className={darkMode ? 'bg-gray-700' : 'bg-gray-100'}>
                            <th
                              className={`text-left px-4 py-2 font-semibold border-b ${darkMode ? 'border-gray-600 text-gray-200' : 'border-gray-200 text-gray-800'}`}
                            >
                              Ingreso
                            </th>
                            <th
                              className={`text-center px-4 py-2 font-semibold border-b ${darkMode ? 'border-gray-600 text-gray-200' : 'border-gray-200 text-gray-800'}`}
                            >
                        ...
                        <tbody>
                          {incomeRules.map((rule) => (
                            <tr key={rule.incomeRange} className={darkMode ? 'border-gray-700' : 'border-gray-200'}>
                              <td
                                className={`px-4 py-3 border-b ${darkMode ? 'border-gray-700 text-gray-300' : 'border-gray-200 text-gray-600'}`}
                              >
                                {rule.incomeRange}
                              </td>
                              <td
              ...
              {category.id !== 'intro' &&
                ...
                guideSections
                  ...
                  .map((section) => (
                    <div key={section.id} className={`rounded-xl border transition-all duration-200 ${cardClass}`}>
                      <button
                      ...
                      {expandedSection === section.id && (
                        <div className="px-6 pb-5 pt-0">
                          <p className="leading-relaxed mb-4 whitespace-pre-line">{section.content}</p>
                          <p
                            className={`text-xs border-t pt-3 ${darkMode ? 'text-gray-500 border-gray-700' : 'text-gray-400 border-gray-200'}`}
                          >
      ...
      {/* Disclaimer */}
      <div
        className={`mt-10 p-6 rounded-xl border-l-4 border-yellow-500 ${darkMode ? 'bg-yellow-900/20' : 'bg-yellow-50'}`}
      >
        ...
        <p className="text-sm">
          Esta guía es solo informativa y no sustituye el asesoramiento fiscal profesional. Los valores y reglas pueden
          cambiar. Consultá siempre con un contador habilitado.
        </p>
      </div>
    </div>
  );
